...
export default class EncryptUtils {
   ...
   */
  static async encrypt(message: string, passKey: string = passkey) {
   ...
   */
  static async decrypt(encryptedMessage: string, passKey: string = passkey) {
    var keyBytes = CryptoJS.PBKDF2(passKey, "Ivan Medvedev", {
      keySize: 48 / 4,
      iterations: 1000,
    });
   ...
   */
  static async encryptURLSafe(value: string) {
    if (!value) {
      return {
